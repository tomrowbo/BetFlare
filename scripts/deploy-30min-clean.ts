...
async function main() {
  ...
  console.log("Deploying 30-min market with:", deployer.address);

  // First, clean up resolved markets from the router
  console.log("\n=== CLEANING UP RESOLVED MARKETS ===");
  const router = await ethers.getContractAt(
    [
      "function getActiveMarkets() view returns (address[])",
      "function deregisterMarket(address)",
      "function registerMarket(address)"
    ],
    CONTRACTS.liquidityRouter
  );

  const activeMarkets = await router.getActiveMarkets();
  ...
  for (const market of activeMarkets) {
    ...
    const isResolved = await fpmm.resolved();
    console.log(`  ${market}: ${isResolved ? "RESOLVED - deregistering" : "active"}`);
    if (isResolved) {
      await (await router.deregisterMarket(market)).wait();
      console.log("    ✓ Deregistered");
    }
  }

  // Market config - resolves in 30 minutes
  const symbol = "testXRP";
  const targetPrice = 150000; // $1.50
  const resolutionTime = Math.floor(Date.now() / 1000) + (30 * 60); // 30 minutes
  const isAbove = true;

  console.log("\n=== 30-MIN MARKET CONFIG ===");
  ...
  const newActiveMarkets = await routerFull.getActiveMarkets();
  console.log("Rebalanced! Active markets:", newActiveMarkets.length);
